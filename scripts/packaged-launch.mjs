...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('Gods Eye.exe');
    await killProcessByName('Gods Eye');
  ...
  const env = {
    ...process.env,
    GODSEYE_EXTENSIONS_PATH: path.join(projectRoot, 'examples'),
  };

  console.log(`[packaged-launch] executable: ${packaged.executablePath}`);
  console.log(`[packaged-launch] cwd: ${packaged.cwd}`);
  console.log(`[packaged-launch] GODSEYE_EXTENSIONS_PATH: ${env.GODSEYE_EXTENSIONS_PATH}`);
