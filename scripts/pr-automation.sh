...
#   LOG_DIR            Directory for log files (default: ~/Library/Logs/Gods Eye)
...
LOG_DIR=${LOG_DIR:-$HOME/Library/Logs/Gods Eye}
...
cleanup_labels() {
  ...
  local stale_wts
  stale_wts=$(find /tmp -maxdepth 1 -name 'godseye-pr-*' -type d 2>/dev/null || true)
  if [ -n "$stale_wts" ]; then
