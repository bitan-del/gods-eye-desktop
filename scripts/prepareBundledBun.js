...
function getRuntimeVersion() {
  const configured = process.env.GODSEYE_BUN_VERSION;
  return configured && configured.trim() ? configured.trim() : 'latest';
}

function getCacheRootDir() {
  const custom = process.env.GODSEYE_BUN_CACHE_DIR;
  ...
  if (process.platform === 'win32') {
    ...
    return path.join(localAppData, 'Gods Eye', 'cache', 'bundled-bun');
  }

  if (process.platform === 'darwin') {
    return path.join(os.homedir(), 'Library', 'Caches', 'Gods Eye', 'bundled-bun');
  }

  const xdgCacheHome = process.env.XDG_CACHE_HOME || path.join(os.homedir(), '.cache');
  return path.join(xdgCacheHome, 'Gods Eye', 'bundled-bun');
}
...
function downloadRuntimeIntoCache(cacheRuntimeDir, platform, arch, version) {
  ...
  const tempRoot = path.join(os.tmpdir(), 'godseye-bundled-bun', version, `${platform}-${arch}`);
