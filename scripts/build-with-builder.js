...
/**
 * Simplified build script for Gods Eye
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const godseyeRunning = isProcessRunningWindows('Gods Eye.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (godseyeRunning || electronRunning) {
        console.log('⚠️  Detected running Gods Eye/Electron process. Attempting to close...');
        killWindowsProcesses(['Gods Eye.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running Gods Eye/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'Gods Eye.exe');
    ...
    console.log('⚠️  Windows local build failed after Gods Eye.exe was produced.');
    if (firstError) {
