...
当前的 `godseye-extension.json` 本质上是一个 **Extension Kit** — 一个扩展可以同时声明 10 种 contribution 类型。但市场化的需求是让用户按 contribution 粒度浏览和发现（"我要找一个翻译助手"、"我要找一个 MCP 服务器"）。
...
flowchart TD
  subgraph Extension["一个 Extension: ext-translator"]
    Manifest["godseye-extension.json"]
...
**LobeHub 能做到纯 JSON 是因为它的 agent 就只是一段 system prompt，无代码、无文件依赖。** Gods Eye 的 contribution 类型更丰富（channelPlugin 需要 JS entryPoint, webui 需要 API routes, theme 需要 CSS 文件），有些天然需要打包在一起，强行拆开反而破坏内聚性。

**关键参考点:**

- LobeHub 的 Agent 市场证明了"按 contribution 粒度展示"的 UX 是成立的
- 但 Gods Eye 不需要照搬"每种类型独立 repo"的模式，因为 contribution 间存在耦合（如 ext-feishu 的 channelPlugin + webui）
