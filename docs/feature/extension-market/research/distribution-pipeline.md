...
> \* **bun 与 npm 的 registry 层完全兼容**：bun 使用相同的 npm registry API (`registry.npmjs.org`)、相同的 .tgz 包格式、支持 `.npmrc` 配置和 `BUN_CONFIG_REGISTRY` 镜像切换。Gods Eye 已内置 bun 运行时，extension 的 `onInstall` 钩子通过 `bun add` 安装 CLI backend。与 npm 的关键差异见 [Q6 补充：bun 运行时注意事项](#bun-运行时注意事项)。
  ...
  "extensions": [
    {
      ...
      "version": "1.2.0",
      "description": "Anthropic Claude Code CLI agent for Gods Eye",
      "author": "godseye",
      ...
      // === 兼容性 ===
      "engines": { "godseye": ">=2.0.0" },
      "dependencies": {},

      // === Hub 自动推导 (从 contributes 提取) ===
      "hubs": ["agent"], // CI 从 godseye-extension.json 的 contributes 自动计算
      "contributes": {
...
Extension 的实际文件 (godseye-extension.json, 脚本, icon 等) 存在哪里？
...
```
godseye/hub/  (GitHub 仓库)
├── extensions/
│   ├── ext-claude-code/
│   │   ├── godseye-extension.json        ← 源文件 (Git 管理)
│   │   ├── install.sh
│   │   ├── icon.png
│   │   └── README.md
│   └── ext-opencode/
...
- Gods Eye 的 extension 不是 JS 包, 发布到 npm 语义不对
...
```
# 版本定义在每个 extension 的 godseye-extension.json 中
{
  "name": "ext-claude-code",
  "version": "1.2.0",              ← extension 版本
  "engines": { "godseye": ">=2.0.0" }  ← 兼容性声明
}
```
...
APP 启动 / 定期 (每 24h)
  ...
  │       AND semver.satisfies(appVersion, remote.engines.godseye)
  │       → 标记为 "可更新"
  │
  └── UI 展示 Update 按钮
```
...
- 后续可加: 在安装新版前备份旧版到 `~/.godseye/extensions/.backup/`, 支持一键回退

**版本号规范**:

- 严格 semver (`major.minor.patch`)
- CI 校验: manifest 中的 `version` 必须大于上一次发布版本 (防止降版本覆盖)
- `engines.godseye` 使用 semver range (如 `>=2.0.0`, `^2.1.0`)
...
async function verifyExtension(tgzBuffer: Buffer, bundleJson: string): Promise<boolean> {
  ...
  await verify(bundle, tgzBuffer, {
    ...
    certificateIdentityRegExp: /https:\/\/github\.com\/godseye\/hub\/.*/,
  });
...
- **JS 库成熟**: `sigstore` npm 包支持 Node.js ≥18 (Gods Eye 的 Electron 满足)
...
const trustedIdentities = [
  /https:\/\/github\.com\/godseye\/hub\/.*/, // 官方仓库
...
└── package/                          ← 固定前缀 (防路径遍历)
    ├── godseye-extension.json           ← manifest
...
客户端下载 extension
  ...
  │   └── 版本匹配 → 直接使用 (零网络)
  │
  ├── 2. 检查本地缓存 (~/.godseye/cache/)
  │   └── 版本匹配 + integrity 验证 → 直接使用
  │
  ├── 3. 从 GitHub 下载 (主源)
  │   ├── 优先: jsDelivr CDN (国内有节点)
  │   │   https://cdn.jsdelivr.net/gh/godseye/hub@main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   ├── 备选: GitHub Raw
  │   │   https://raw.githubusercontent.com/godseye/hub/main/dist/ext-claude-code-1.2.0.tgz
  │   │
  │   └── 备选: GitHub Releases (如果迁移到 Releases)
  │       https://github.com/godseye/hub/releases/download/v2026.03/ext-claude-code-1.2.0.tgz
  │
  ├── 4. 下载后
  │   ├── 验证 SHA-512 integrity
  │   ├── 验证 Sigstore 签名 (Phase 2)
  │   ├── 缓存到 ~/.godseye/cache/
  │   └── 解压到 ~/.godseye/extensions/ext-claude-code/
  │
  └── 5. 所有源失败
      └── 显示错误提示, 建议检查网络
```
...
下载 .tgz 完成
  ...
  │
  ├── 3. 解压到临时目录
  │   └── ~/.godseye/extensions/.tmp/ext-claude-code/
  │
  ├── 4. 二次校验
  │   ├── godseye-extension.json 存在且合法
  │   ├── 路径安全检查 (无 symlink 逃逸)
  │   └── engines 兼容性检查
  │
  ├── 5. 原子替换
...
Gods Eye 内置了 bun 运行时，extension 的 `onInstall` 钩子通过 `bun add` 安装 CLI backend（如 `bun add -g @anthropic-ai/claude-code`）。bun 与 npm 共用 registry API，但有以下实际差异需要在实现中处理：
...
```mermaid
flowchart TD
  subgraph GitHub["GitHub (godseye/hub 仓库)"]
    direction TB
    ExtDirs["extensions/\next-claude-code/\next-opencode/\n..."]
  ...
  subgraph App["Gods Eye APP"]
    direction TB
    Bundled["内置资源\ndist/ 快照\n(打包进 APP)"]
    IndexMerge["Index 合并器\n内置 + 远程"]
    Cache["本地缓存\n~/.godseye/cache/"]
    Installer["Extension 安装器\n下载 → 验证 → 解压 → 激活"]
    ExtDir["~/.godseye/extensions/\n已安装 extension"]
