...
| 权限         | 值                   | 风险等级  | 说明                                      |
| ------------ | -------------------- | --------- | ----------------------------------------- |
| `storage`    | `true`               | safe      | 读写 Gods Eye 持久化存储                    |
...
> 消息路由修复详情见 [sandbox-architecture.md](sandbox-architecture.md) 和 PR [#1991](https://github.com/bitan-del/gods-eye/pull/1991)。
...
1. ~~**`godseye.storage` API 不可用**~~ — `handleMessage()` 已加 `case 'api-call'`，通过 `apiHandlers` map 路由到具体实现，回 `api-response`。无 handler 时回 error response 而不是静默丢弃。
...
| 类型                      | 隔离方式                                             | 安全属性            |
| ------------------------- | ---------------------------------------------------- | ------------------- |
| 本地 HTML (godseye-asset://) | `<iframe sandbox='allow-scripts allow-same-origin'>` | 脚本执行 + 同源访问 |
...
- 不经过 Worker Thread Sandbox，无 `godseye` API 可用
