...
flowchart TD
  ...
  subgraph scan["目录扫描 (优先级递减)"]
    S1["1. GODSEYE_EXTENSIONS_PATH\n环境变量 (可多路径)"]
    S2["2. ~/.godseye/extensions/\n用户目录"]
...
│   ├── sandboxWorker.ts              Worker 脚本: godseye API proxy + callMainThread
│   ├── ExtensionStorage.ts           扩展 KV 存储 (JSON 文件, 按扩展隔离)
│   ├── permissions.ts                权限分析、风险等级分类
│   └── pathSafety.ts                 路径遍历防护
├── protocol/
│   ├── assetProtocol.ts              godseye-asset:// 自定义协议
...
└── resolvers/
    ...
    └── utils/
        ...
        ├── engineValidator.ts        Gods Eye 版本 + API 版本兼容性
