...
> 目的：深度调研 Gods Eye Extension 系统现状，为 Agent Hub / Assistant Hub / MCP Hub / Skill Hub 等市场化建设提供基础
...
当前的 `godseye-extension.json` 允许一个扩展同时声明 10 种 contribution 类型。这本质上是一个 **Extension Kit** 而非原子 Extension。例如 `hello-world-extension` 同时贡献了 acpAdapters、mcpServers、assistants、agents、skills、themes、settingsTabs 七种类型。
...
Extension 系统是 Gods Eye 的插件化基础设施，允许第三方通过声明式 manifest (`godseye-extension.json`) 向 Gods Eye 贡献多种能力类型。系统横跨 Electron 主进程、渲染进程和 Worker Thread 三层。

### Manifest 结构 (`godseye-extension.json`)

```
godseye-extension.json
├── name          (kebab-case, 2-64 chars, 不可用 godseye-/internal-/builtin-/system- 前缀)
├── displayName
├── version       (semver)
├── description?, author?, icon?, homepage?
├── apiVersion?   (^semver, API 兼容性声明)
├── engine?       { godseye: "^x.y.z" }
...
- Worker Thread 沙箱的消息路由已修复（PR [#1991](https://github.com/bitan-del/gods-eye/pull/1991)：Worker→Host RPC、事件转发、storage API），但 `createSandbox()` 尚未被实际调用（ChannelPlugin 和 Lifecycle hooks 仍在主进程裸跑，待迁移）。
