...
> PR：[#1991](https://github.com/bitan-del/gods-eye/pull/1991)
...
这是 **Worker → Host** 方向的 RPC，扩展通过 `godseye.storage.*` 调用主进程服务。
...
sequenceDiagram
  ...
  Note over E,S: 扩展调用 godseye.storage.get('key')

  E->>W: godseye.storage.get('key')
...
> Worker 发来的消息落入 `default: break` 被静默丢弃，导致 `godseye.storage.*` 调用永远 hang。
...
扩展通过 `godseye.emitEvent()` 和 `godseye.postToUI()` 向外发送事件，
...
sequenceDiagram
  ...
  Note over E,Bus: 扩展调用 godseye.emitEvent('data-ready', payload)

  E->>W: godseye.emitEvent('data-ready', payload)
...
sequenceDiagram
  ...
  Note over E,R: 扩展调用 godseye.postToUI(data)

  E->>W: godseye.postToUI(data)
...
│ Worker Thread (扩展代码)                                                    │
│                                                                             │
│  godseye.postToUI(data)                   godseye.emitEvent('data-ready', data)   │
│       │                                      │                              │
│       ▼                                      ▼                              │
...
```
  ...
  │  → apiHandlers 路由      │              │  godseye.storage.get/set    │
  │  → 回 api-response       │──api-resp──→ │  pendingMainCalls.resolve│
  │  ✓ 已修复 (PR 1991)      │              │  ✓ 30s 超时保护          │
  │                          │              │                          │
  │  case 'event' 路由:      │←───event──── │  godseye.emitEvent()        │
  │  ext:* → eventBus        │              │  godseye.postToUI()         │
