...
> PR：[#1991](https://github.com/bitan-del/gods-eye/pull/1991) (问题 1-3)
> Issue：[#1990](https://github.com/bitan-del/gods-eye/issues/1990)
...
扩展代码调用 `godseye.storage.get/set/delete` 后永远不返回，整个扩展代码卡死。
...
JSON 文件 KV 存储，每个扩展一个文件 `~/.godseye/extension-storage/{name}.json`：
...
扩展调用 `godseye.emitEvent()` 或 `godseye.postToUI()`，事件消息从未到达 `extensionEventBus` 或渲染进程。
