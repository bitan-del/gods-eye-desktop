...
Gods Eye 设置页 > Agent > Local Agent 页，APP 启动时自动扫描本地已安装的 CLI backend（Claude Code、Opencode、iflow、Kimi 等），将检测到的 agent 展示在列表中供用户使用。
...
3. **不知道有哪些可用**：用户不知道 Gods Eye 支持哪些 agent backend，缺少发现入口
...
```
Hub 中展示的 Agent ──── 对应 ──── 一个 Extension (godseye-extension.json)
...
| **远程** | 从 GitHub `godseye/hub` 仓库拉取的 index.json 中，存在于远程但不在内置列表中的 extension | 需要网络 |
...
```json
{
  ...
  "extensions": {
    "godseyeext-auggie": {
      "name": "godseyeext-auggie",
      "displayName": "Augment Code",
      "version": "1.0.0",
      "description": "Integrates Augment Code as an ACP adapter in Gods Eye.",
      ...
      "hubs": ["acpAdapters"],
      "dist": {
        "tarball": "dist/godseyeext-auggie.zip",
        "integrity": "sha512-F7Lyl3J3T/e4kkOoADUxw9YPuI5ljeIGiJOb1RQOvm4KztCgpB+hUyBECan+1T4exikU5F9ry9wEssJUGvW/nw==",
        "unpackedSize": 549
      }
    },
    "godseyeext-claude": {
      "name": "godseyeext-claude",
      "displayName": "Claude Code",
      "version": "1.0.0",
      "description": "Integrates Anthropic's Claude Code CLI as an ACP adapter in Gods Eye.",
      "author": "Aionui Official",
      "engines": {
        "godseye": "^1.0.0"
      },
      "hubs": ["acpAdapters"],
      "dist": {
        "tarball": "dist/godseyeext-claude.zip",
...
`engines.godseye` 定义了兼容的 Gods Eye 版本范围，
...
判断 Hub extension 的展示状态:
  ...
  extension 目录不存在 (~/.godseye/extensions/godseyeext-xxx/)
...
| 状态信号           | 数据来源                                         | 说明           |
| ------------------ | ------------------------------------------------ | -------------- |
| extension 是否存在 | `ExtensionLoader` 扫描 `~/.godseye/extensions/`   | 已有能力       |
...
APP 启动
  │
  ├── 1. ExtensionLoader 扫描 ~/.godseye/extensions/ (现有逻辑)
  │      → 得到已安装 extension 列表
  │
...
用户点击 Install
  ...
  │
  ├── 4. 解压到临时目录
  │      └── ~/.godseye/extensions/.tmp/godseyeext-xxx/
  │
  ├── 5. 校验 godseye-extension.json 合法性
  │      └── 失败 → 清理临时目录, 状态 → install_failed
  │
  ├── 6. 原子移动到正式目录
  │      └── .tmp/godseyeext-xxx/ → ~/.godseye/extensions/godseyeext-xxx/
  │
...
用户点击 Uninstall (更多菜单中)
  ...
  │      └── 失败 → 仅 warn 日志, 不阻塞卸载
  │
  ├── 5. 删除 ~/.godseye/extensions/godseyeext-xxx/ 目录
  │
...
用户点击 Retry (在 install_failed 状态)
  │
  ├── 检查 ~/.godseye/extensions/godseyeext-xxx/ 是否已存在
...
│   ├── godseyeext-claude.tgz      ← 打包后的 extension
│   ├── godseyeext-opencode.tgz
│   └── ...
├── scripts/
...
```
Gods Eye.app/Contents/Resources/
└── hub/
    ├── index.json                   ← 构建 APP 时从 godseye/hub 的 dist/ 复制
    ├── godseyeext-claude.tgz           ← 内置 extension (bundled: true)
    ├── godseyeext-kimi.tgz
    └── godseyeext-opencode.tgz
```
...
拉取 remote-index.json
  ├── 优先: GitHub Raw
  │   https://raw.githubusercontent.com/godseye/hub/main/dist/index.json
  │
  ├── 备选: jsDelivr CDN
  │   https://cdn.jsdelivr.net/gh/godseye/hub@main/dist/index.json
  │
  ├── 超时: 5s (不阻塞 APP 启动)
  ├── 重试: 1 次
  │
...
for (const hubAgent of hubAgents) {
  ...
  const extDir = `~/.godseye/extensions/${hubAgent.name}/`;
