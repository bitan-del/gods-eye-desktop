# Gods Eye WebUI Mode - Startup Guide

Gods Eye supports WebUI mode, allowing you to access the application through a web browser. This guide covers how to start WebUI mode on all supported platforms.
...
WebUI mode starts Gods Eye with an embedded web server, allowing you to:

- Access the application through any modern web browser
- Use Gods Eye from remote devices on the same network (with `--remote` flag)
...
```cmd
# Using full path
"C:\Program Files\Gods Eye\Gods Eye.exe" --webui

# Or if Gods Eye is in your PATH
Gods Eye.exe --webui
```
...
```bash
# Using full path
/Applications/Gods Eye.app/Contents/MacOS/Gods Eye --webui

# Or using open command
open -a Gods Eye --args --webui
```

### Method 2: Create Shell Script

Create `start-godseye-webui.sh`:
...
```bash
chmod +x start-godseye-webui.sh
./start-godseye-webui.sh
```
...
chmod +x Gods Eye-*.AppImage

# Run with --webui flag
./Gods Eye-*.AppImage --webui
```

### Method 2: Create Desktop Entry

Create `~/.local/share/applications/godseye-webui.desktop`:

```ini
[Desktop Entry]
Name=Gods Eye WebUI
Comment=Start Gods Eye in WebUI mode
Exec=/opt/Gods Eye/godseye --webui
Icon=godseye
Terminal=false
Type=Application
...
```bash
chmod +x ~/.local/share/applications/godseye-webui.desktop
```
...
Create `~/bin/start-godseye-webui.sh`:
...
```bash
chmod +x ~/bin/start-godseye-webui.sh
```
...
Create `/etc/systemd/system/godseye-webui.service`:

```ini
[Unit]
Description=Gods Eye WebUI Service
After=network.target

[Service]
Type=simple
User=YOUR_USERNAME
ExecStart=/opt/Gods Eye/godseye --webui --remote
Restart=on-failure
RestartSec=10
...
sudo systemctl enable godseye-webui.service
sudo systemctl start godseye-webui.service

# Check status
sudo systemctl status godseye-webui.service
```
...
**Important Note**: Electron desktop mode is **not supported** on Android. However, you can run Gods Eye in WebUI mode using Termux with a prooted Linux environment.

> **Community Contribution**: This guide is contributed by [@Manamama](https://github.com/Manamama). Special thanks for making Gods Eye accessible on Android devices! 🙏
>
> **Original Tutorial**: [Running Gods Eye WebUI on Android via Termux + Proot Ubuntu](https://gist.github.com/Manamama/b4f903c279b5e73bdad4c2c0a58d5ddd)
>
> **Related Issues**: [#217 - Android Support Discussion](https://github.com/bitan-del/gods-eye/issues/217)
...
#### 4. Download and Install Gods Eye

```bash
# Download the ARM64 .deb package (replace VERSION with the actual version)
# Check latest version at: https://github.com/bitan-del/gods-eye/releases
wget https://github.com/bitan-del/gods-eye/releases/download/vVERSION/Gods Eye_VERSION_arm64.deb

# Example (replace VERSION with the release tag, e.g. v1.5.2):
wget https://github.com/bitan-del/gods-eye/releases/download/vVERSION/Gods Eye_VERSION_arm64.deb

# Install the package
apt install -y ./Gods Eye_*.deb

# Verify installation
which Gods Eye
```

#### 5. Launch Gods Eye WebUI

```bash
# Start Gods Eye in WebUI mode with no-sandbox flag
Gods Eye --no-sandbox --webui
```
...
To access Gods Eye from other devices on your local network:

```bash
# Start with --remote flag
Gods Eye --no-sandbox --webui --remote
...
Gods Eye --no-sandbox --webui --port 8080
```
...
chmod +x /opt/Gods Eye/godseye
```

#### Out of Memory

Gods Eye requires sufficient RAM. Close other apps if you encounter memory issues.

#### Cannot Access from Browser

1. Check if Gods Eye is running: look for "Server started" message
...
4. **Keep device charged** - Running Gods Eye consumes battery

### Tested Environment

- **Device**: Android 14
- **Termux Version**: 0.118.0
- **Gods Eye Version**: Latest release (e.g. 1.5.2)
- **Proot-distro**: Ubuntu (latest)

### Creating a Startup Script

For convenience, create a script to launch Gods Eye quickly:

```bash
# Create script in Ubuntu (proot)
cat > ~/start-godseye.sh << 'EOF'
#!/bin/bash
echo "Starting Gods Eye WebUI..."
Gods Eye --no-sandbox --webui --remote
EOF

# Make executable
chmod +x ~/start-godseye.sh
...
```bash
proot-distro login ubuntu -- bash -c "Gods Eye --no-sandbox --webui --remote"
```
...
2. Report issues at [GitHub Issues #217](https://github.com/bitan-del/gods-eye/issues/217)
...
```cmd
Gods Eye.exe --webui --remote
```

### macOS

```bash
/Applications/Gods Eye.app/Contents/MacOS/Gods Eye --webui --remote
```

### Linux

```bash
godseye --webui --remote
```
...
netsh advfirewall firewall add rule name="Gods Eye WebUI" dir=in action=allow protocol=TCP localport=3000
```
...
**macOS:**
Go to **System Preferences** → **Security & Privacy** → **Firewall** → **Firewall Options** → Add Gods Eye
...
which godseye
# or
find /opt -name "godseye" 2>/dev/null
```
...
```powershell
& "C:\Program Files\Gods Eye\Gods Eye.exe" --webui 2>&1 | Tee-Object -FilePath godseye.log
```

**macOS/Linux:**

```bash
/path/to/godseye --webui 2>&1 | tee godseye.log
```
...
export GODSEYE_PORT=8080

# Allow remote access without passing --remote
export GODSEYE_ALLOW_REMOTE=true

# Optional host hint (0.0.0.0 behaves the same as GODSEYE_ALLOW_REMOTE=true)
export GODSEYE_HOST=0.0.0.0

# Then start the application
godseye --webui

# You can also pass the port directly via CLI
godseye --webui --port 8080
```
...
```cmd
# Using full path
"C:\Program Files\Gods Eye\Gods Eye.exe" --resetpass

# Or for a specific user
"C:\Program Files\Gods Eye\Gods Eye.exe" --resetpass username
```
...
```bash
# Using full path
/Applications/Gods Eye.app/Contents/MacOS/Gods Eye --resetpass

# Or for a specific user
/Applications/Gods Eye.app/Contents/MacOS/Gods Eye --resetpass username
```
...
# Or for a specific user
godseye --resetpass username

# Or using full path
/opt/Gods Eye/godseye --resetpass
```
...
- [GitHub Issues](https://github.com/bitan-del/gods-eye/issues)
...
2. Search [existing issues](https://github.com/bitan-del/gods-eye/issues)
3. Create a [new issue](https://github.com/bitan-del/gods-eye/issues/new) with:
   - Your OS and version
   - Gods Eye version
   - Steps to reproduce
...
**Happy using Gods Eye in WebUI mode!** 🚀
