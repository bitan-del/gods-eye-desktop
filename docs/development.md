...
When you have two clones of the repository (e.g. `Gods Eye` and `Gods Eye-refactor`) and need to run both simultaneously, the second instance can be started with:

```bash
bun run start:multi
```

This sets `GODSEYE_MULTI_INSTANCE=1`, which:

- Skips the Electron single-instance lock
- Uses a separate userData directory (`Gods Eye-Dev-2`) to avoid database and config conflicts
- Isolates data/config symlink paths (`~/.godseye-dev-2`, `~/.godseye-config-dev-2`)
...
Gods Eye uses **electron-vite** for fast bundling:
