 ...
 *      (e.g. out/linux-unpacked/godseye, out/mac-arm64/Gods Eye.app, out/win-unpacked/Gods Eye.exe).
...
const e2eStateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'godseye-e2e-state-'));
...
function resolvePackagedApp(): { executablePath: string; cwd: string } | null {
  ...
  if (platform === 'win32') {
    // out/win-unpacked/Gods Eye.exe  or  out/win-x64-unpacked/Gods Eye.exe
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'Gods Eye.exe');
      if (fs.existsSync(exe)) return { executablePath: exe, cwd: path.join(outDir, dir) };
    }
  } else if (platform === 'darwin') {
    // out/mac-arm64/Gods Eye.app/Contents/MacOS/Gods Eye  or  out/mac/Gods Eye.app/...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      if (appBundle) {
        const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'Gods Eye');
        if (fs.existsSync(exe)) return { executablePath: exe, cwd: macDir };
      }
    }
  } else {
    // Linux: out/linux-unpacked/godseye  (lowercase executable name)
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      const dirPath = path.join(outDir, dir);
      if (!fs.existsSync(dirPath)) continue;
      // Try common executable names
      for (const name of ['godseye', 'Gods Eye']) {
...
async function launchApp(): Promise<ElectronApplication> {
  ...
  const commonEnv = {
    ...process.env,
    GODSEYE_EXTENSIONS_PATH: process.env.GODSEYE_EXTENSIONS_PATH || path.join(projectRoot, 'examples'),
    GODSEYE_EXTENSION_STATES_FILE: process.env.GODSEYE_EXTENSION_STATES_FILE || e2eStateFile,
    GODSEYE_DISABLE_AUTO_UPDATE: '1',
    GODSEYE_DISABLE_DEVTOOLS: '1',
    GODSEYE_E2E_TEST: '1',
    GODSEYE_CDP_PORT: '0',
  };
