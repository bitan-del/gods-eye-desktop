...
const emptyExtensionsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'godseye-e2e-no-extensions-'));
const stateSandboxDir = fs.mkdtempSync(path.join(os.tmpdir(), 'godseye-e2e-no-extensions-state-'));
...
async function launchAppWithoutExtensions(): Promise<ElectronApplication> {
  ...
  return electron.launch({
    ...
    env: {
      ...process.env,
      GODSEYE_EXTENSIONS_PATH: emptyExtensionsDir,
      GODSEYE_EXTENSION_STATES_FILE: extensionStatesFile,
