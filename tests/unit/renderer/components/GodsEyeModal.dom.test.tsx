...
vi.mock('@icon-park/react', () => ({
  Close: ({ size = 20 }: { size?: number }) => <span data-testid='godseye-modal-close' data-size={size} />,
}));
...
describe('GodsEyeModal', () => {
  it('uses dialog fill as the default content background', () => {
    const { container } = render(
      <GodsEyeModal visible onCancel={vi.fn()} header='Modal title'>
        content
      </GodsEyeModal>
    );

    const body = container.querySelector('.godseye-modal-body-content');
