...
vi.mock('@process/extensions/constants', () => ({
  EXTENSION_MANIFEST_FILE: 'godseye-extension.json',
  ...
  describe('install', () => {
    ...
    it('should use bundled zip when available', async () => {
      ...
      mockedExistsSync.mockImplementation((p) => {
        ...
        if (s.includes('godseye-extension.json')) return true;
        return false;
      });
    ...
    it('should download from remote when not bundled', async () => {
      ...
      mockedExistsSync.mockImplementation((p) => String(p).includes('godseye-extension.json'));
    ...
    it('should fail when manifest is missing after extraction', async () => {
      ...
      await expect(hubInstaller.install('bad-pkg')).rejects.toThrow('godseye-extension.json missing');
    });
  });
