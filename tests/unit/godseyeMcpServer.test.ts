 ...
 *   - godseye_create_team: input validation, TeamSessionService wiring, return shape
 *   - godseye_navigate: route whitelist enforcement, IPC emit
...
function getPort(service: AionMcpService): number {
  const entry = service.getStdioConfig().env.find((e) => e.name === 'GODSEYE_MCP_PORT');
  return Number(entry?.value ?? 0);
}

function getAuthToken(service: AionMcpService): string {
  return service.getStdioConfig().env.find((e) => e.name === 'GODSEYE_MCP_TOKEN')?.value ?? '';
}
...
describe('AionMcpService lifecycle', () => {
  ...
  it('getStdioConfig returns correct structure', () => {
    ...
    expect(config.name).toBe('godseye-team-guide');
    expect(config.command).toBe('node');
    expect(Array.isArray(config.args)).toBe(true);
    expect(config.env.some((e) => e.name === 'GODSEYE_MCP_PORT')).toBe(true);
    expect(config.env.some((e) => e.name === 'GODSEYE_MCP_TOKEN')).toBe(true);
  });
  ...
  it('GODSEYE_MCP_PORT resets to 0 after stop', async () => {
    await service.stop();
...
// ------------------------------------------------------------------
// godseye_create_team handler
// ------------------------------------------------------------------

describe('godseye_create_team handler', () => {
  let service: AionMcpService;
  ...
  it('uses system-injected backend (from GODSEYE_MCP_BACKEND) as agent type', async () => {
    await tcpRequest(getPort(service), {
      tool: 'godseye_create_team',
      args: { summary: '分析代码' },
      auth_token: getAuthToken(service),
...
// ------------------------------------------------------------------
// godseye_navigate handler
// ------------------------------------------------------------------

describe('godseye_navigate handler', () => {
  let service: AionMcpService;
