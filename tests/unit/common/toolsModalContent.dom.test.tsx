...
const testState = vi.hoisted(() => ({
  ...
  initialBuiltinServer: (): MockServer => ({
    ...
    transport: {
      ...
      env: {
        GODSEYE_IMG_PLATFORM: 'new-api',
        GODSEYE_IMG_BASE_URL: 'https://example.com',
        GODSEYE_IMG_API_KEY: 'key',
        GODSEYE_IMG_MODEL: 'grok-imagine-1.0',
      },
    },
  }),
  mockConfigGet: vi.fn(),
...
vi.mock('@/renderer/components/base/GodsEyeSelect', () => {
  const Select = ({
    children,
    value,
    onChange,
...
describe('ToolsModalContent image generation status refresh', () => {
  ...
  it('refreshes image generation agent status only after sync completes when enabling the builtin MCP server', async () => {
    ...
    expect(testState.mockCheckSingleServerInstallStatus).toHaveBeenCalledWith('godseye-image-generation');
  });
