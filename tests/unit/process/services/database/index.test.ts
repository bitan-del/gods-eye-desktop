...
import { GodsEyeDatabase } from '@process/services/database/index';
...
describe('GodsEyeDatabase.create recovery', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it('closes the failed driver before attempting file recovery', async () => {
    ...
    await GodsEyeDatabase.create('/tmp/test.db');
  ...
  it('recovers successfully after closing the failed driver', async () => {
    ...
    const db = await GodsEyeDatabase.create('/tmp/test.db');
    expect(db).toBeInstanceOf(GodsEyeDatabase);
  ...
  it('does not close driver when createDriver itself throws', async () => {
    ...
    await expect(GodsEyeDatabase.create('/tmp/test.db')).rejects.toThrow('dlopen');
  });
  ...
  it('throws when corrupted file cannot be renamed or deleted', async () => {
    ...
    await expect(GodsEyeDatabase.create('/tmp/test.db')).rejects.toThrow(
