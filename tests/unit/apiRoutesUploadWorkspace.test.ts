...
describe('resolveUploadWorkspace', () => {
  ...
  it('returns the stored conversation workspace when requested workspace matches', async () => {
    getConversation.mockReturnValue({
      ...
      data: {
        extra: {
          workspace: '/tmp/godseye/workspace-1',
        },
      },
    });

    const resolved = await resolveUploadWorkspace('conv-1', '/tmp/godseye/workspace-1');

    expect(resolved).toBe(path.resolve('/tmp/godseye/workspace-1'));
  ...
  it('allows uploads without a requested workspace and still uses the stored conversation workspace', async () => {
    getConversation.mockReturnValue({
      ...
      data: {
        extra: {
          workspace: '/tmp/godseye/workspace-2',
        },
      },
    });

    await expect(resolveUploadWorkspace('conv-2')).resolves.toBe(path.resolve('/tmp/godseye/workspace-2'));
  });
  ...
  it('rejects uploads when the requested workspace does not match the conversation workspace', async () => {
    getConversation.mockReturnValue({
      ...
      data: {
        extra: {
          workspace: '/tmp/godseye/workspace-3',
        },
      },
    });
