...
vi.mock('@/renderer/components/base/GodsEyeModal', () => ({
  default: ({
    children,
    visible,
    contentStyle,
    ...
    visible ? (
      <div data-testid='godseye-modal' data-background={contentStyle?.background ?? ''}>
        {children}
      </div>
    ) : null,
}));
...
describe('LocalAgents', () => {
  ...
  it('uses the shared dialog surface for the custom agent editor modal', async () => {
    ...
    expect(screen.getByTestId('godseye-modal')).toHaveAttribute('data-background', 'var(--dialog-fill-0)');
