...
// from running Gods Eye instances, causing port conflicts. Back it up and restore.
const REAL_REGISTRY = path.join(os.homedir(), '.godseye-cdp-registry.json');
let savedRegistry: string | null = null;

function createSandbox(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'godseye-cdp-test-'));
}
...
async function loadConfigureChromium(options: SetupOptions = {}) {
  ...
  const registryPath = path.join(sandbox, '.godseye-cdp-registry.json');
  ...
  delete process.env.GODSEYE_CDP_PORT;
  if (options.envPort !== undefined) {
    process.env.GODSEYE_CDP_PORT = options.envPort;
  }
...
describe('configureChromium CDP (lightweight mock + file sandbox)', () => {
  ...
  describe('dev environment isolation', () => {
    it('sets app name and userData path in dev mode', async () => {
      ...
      expect(ctx.setNameSpy).toHaveBeenCalledWith('Gods Eye-Dev');
      expect(ctx.setPathSpy).toHaveBeenCalledWith('userData', path.join(ctx.sandbox, 'Gods Eye-Dev'));
    });
