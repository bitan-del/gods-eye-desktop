...
describe('prepareBundledBun', () => {
  ...
  const originalCacheDir = process.env.GODSEYE_BUN_CACHE_DIR;
  const originalVersion = process.env.GODSEYE_BUN_VERSION;
  ...
  afterEach(() => {
    process.env.GODSEYE_BUN_CACHE_DIR = originalCacheDir;
    process.env.GODSEYE_BUN_VERSION = originalVersion;
  ...
  it('copies bundled bun from cache when cache metadata is valid', () => {
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'godseye-bun-test-'));
    ...
    process.env.GODSEYE_BUN_CACHE_DIR = cacheRoot;
    process.env.GODSEYE_BUN_VERSION = version;
