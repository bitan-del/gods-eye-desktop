...
vi.mock('electron', () => ({
  app: {
    isPackaged: false,
    getPath: vi.fn(() => '/tmp/godseye-test'),
  },
}));
...
function createExtension(baseDir: string, folderName: string, manifestName: string, version = '1.0.0'): void {
  ...
  fs.writeFileSync(
    path.join(extensionDir, 'godseye-extension.json'),
    JSON.stringify(
      {
        name: manifestName,
...
describe('extensions/ExtensionLoader', () => {
  it('prioritizes explicit env extensions over duplicate user-installed extensions and skips implicit examples', async () => {
    const sandbox = createTempDir('godseye-loader-');
    ...
    process.env.GODSEYE_EXTENSIONS_PATH = envDir;

    createExtension(path.join(homeDir, '.godseye-dev', 'extensions'), 'ext-shadow', 'ext-shadow', '1.0.0');
  ...
  it('keeps E2E discovery hermetic by ignoring user, appdata, and implicit example sources', async () => {
    const sandbox = createTempDir('godseye-loader-e2e-');
    const homeDir = path.join(sandbox, 'home');
    const envDir = path.join(sandbox, 'env-extensions');
    const projectRoot = path.join(sandbox, 'project');
    const appDataExtensionsDir = path.join(homeDir, 'AppData', 'Roaming', 'Gods Eye', 'extensions');
    ...
    createExtension(path.join(homeDir, '.godseye-dev', 'extensions'), 'user-only', 'user-only');
