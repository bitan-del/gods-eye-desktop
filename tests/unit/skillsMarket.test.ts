 ...
 *
 * Tests the enable/disable flow for the godseye-skills builtin skill:
...
const BUNDLED_SKILL_PATH = path.resolve(
  __dirname,
  '../../src/process/resources/skills/_builtin/godseye-skills/SKILL.md'
);

// Mock Electron app and initStorage before importing AcpSkillManager
vi.mock('electron', () => ({ app: { setName: vi.fn(), getPath: () => '/tmp/godseye-test' } }));
vi.mock('../../src/process/utils/initStorage', () => ({
  getSkillsDir: () => path.join('/tmp/godseye-test', 'skills'),
  getAutoSkillsDir: () => path.join('/tmp/godseye-test', 'skills', '_builtin'),
  getBuiltinSkillsCopyDir: () => path.join('/tmp/godseye-test', 'builtin-skills'),
}));
...
describe('Skills Market - Bundled SKILL.md', () => {
  ...
  it('has valid frontmatter with name and description', async () => {
    ...
    expect(nameMatch![1].trim()).toBe('godseye-skills');

    const descMatch = frontmatter.match(/^description:\s*(.+)$/m);
    expect(descMatch).not.toBeNull();
    expect(descMatch![1]).toContain('Gods Eye Skills');
  });

  it('contains the curl command for fetching full SKILL.md', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('curl -s https://skills.gods-eye.org/SKILL.md');
  });
  ...
  it('references the standard credentials path', async () => {
    const content = await fs.readFile(BUNDLED_SKILL_PATH, 'utf-8');
    expect(content).toContain('~/.config/godseye-skills');
  });
  ...
  it('does NOT contain the full API documentation', async () => {
    ...
    expect(content).not.toContain('X-Gods Eye-Skills-Checksum');
  });
});
...
describe('Skills Market - Enable/Disable flow', () => {
  ...
  it('enable: creates godseye-skills directory with SKILL.md', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'godseye-skills');
    ...
    expect(written).toContain('name: godseye-skills');
  });

  it('disable: removes godseye-skills directory completely', async () => {
    const builtinDir = path.join(tmpDir, '_builtin');
    const skillDir = path.join(builtinDir, 'godseye-skills');
  ...
  it('disable: fs.rm with force does not throw if directory does not exist', async () => {
    const skillDir = path.join(tmpDir, '_builtin', 'godseye-skills');
