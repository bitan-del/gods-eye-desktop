...
describe('buildDisplayMessage', () => {
  const workspace = '/tmp/godseye/workspace-1';
  ...
  it('strips GODSEYE timestamp separators from filenames while keeping prefix', () => {
    const files = [`${workspace}/uploads/photo_godseye_1234567890123.jpg`];
    const result = buildDisplayMessage('hello', files, workspace);
    expect(result).toContain(`${workspace}/uploads/photo.jpg`);
  });
});
