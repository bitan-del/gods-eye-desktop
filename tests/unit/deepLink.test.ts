...
describe('deepLink module', () => {
  ...
  describe('parseDeepLinkUrl', () => {
    it('should parse simple deep link URL', async () => {
      ...
      const result = parseDeepLinkUrl('godseye://add-provider?baseUrl=http://localhost&apiKey=sk-123');
    ...
    it('should parse deep link with path segments', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
      const result = parseDeepLinkUrl('godseye://provider/add?v=1');
    ...
    it('should decode base64 data param and merge into params', async () => {
      ...
      const result = parseDeepLinkUrl(`godseye://provider/add?v=1&data=${data}`);
    ...
    it('should handle invalid base64 data gracefully', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
      const result = parseDeepLinkUrl('godseye://add?data=not-valid-base64!!!');

      expect(result).not.toBeNull();
      expect(result!.params.data).toBeUndefined();
    });

    it('should return null for non-godseye protocol', async () => {
      const { parseDeepLinkUrl } = await import('@process/utils/deepLink');
  ...
  describe('handleDeepLinkUrl', () => {
    it('should queue URL when no window is set', async () => {
      ...
      handleDeepLinkUrl('godseye://test-action?key=val');

      expect(getPendingDeepLinkUrl()).toBe('godseye://test-action?key=val');
    });
  ...
  describe('pending URL state', () => {
    it('should clear pending URL', async () => {
      ...
      handleDeepLinkUrl('godseye://test');
      expect(getPendingDeepLinkUrl()).toBe('godseye://test');
