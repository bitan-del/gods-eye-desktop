...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    renderer: {
      ...
      appType: 'mpa',
      server: {
        // Default to 5173; when occupied (e.g. another Gods Eye clone is running),
      ...
      define: {
        'process.env.NODE_ENV': JSON.stringify(mode),
        'process.env.env': JSON.stringify(process.env.env),
        'process.env.GODSEYE_MULTI_INSTANCE': JSON.stringify(process.env.GODSEYE_MULTI_INSTANCE ?? ''),
        'process.env.SENTRY_DSN': JSON.stringify(process.env.SENTRY_DSN ?? ''),
        global: 'globalThis',
      },
      optimizeDeps: {
