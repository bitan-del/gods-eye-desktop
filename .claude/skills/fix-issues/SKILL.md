...
```
~/.godseye-fix-issues/skip-list.json
```
...
```bash
gh api repos/bitan-del/gods-eye/issues/<number>/comments \
...
```bash
gh issue list --repo bitan-del/gods-eye --state open --label bug --limit 50 \
...
gh api repos/bitan-del/gods-eye/issues/<number>/timeline --jq \
...
```bash
gh issue comment <number> --repo bitan-del/gods-eye --body "$(cat <<'EOF'
...
```bash
gh pr list --repo bitan-del/gods-eye --state open --search "<error-keyword-or-file>" --json number,title
```
...
In daemon mode (`limit > 0`), after the summary report, update `~/.godseye-fix-issues/skip-list.json`
...
| --------- | ---------------- | ------------------------------------------------------------------- |
| repo      | bitan-del/gods-eye | GitHub repository (owner/repo)                                      |
