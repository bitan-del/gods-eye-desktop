...
# Check open PRs
gh pr list --repo bitan-del/gods-eye --state open --search "<error-keyword>" --json number,title

# Check closed PRs (recently merged)
gh pr list --repo bitan-del/gods-eye --state merged --search "<error-keyword>" --limit 5 --json number,title,mergedAt
```
