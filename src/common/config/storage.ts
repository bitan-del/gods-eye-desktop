...
export interface IConfigStorageRefer {
  ...
  // Skills Market: whether the godseye-skills builtin skill is enabled
 ...
 * 会话来源类型 - 标识会话创建的来源
 */
export type ConversationSource = 'godseye' | 'telegram' | 'lark' | 'dingtalk' | 'weixin' | 'wecom' | (string & {});
...
interface IChatConversation<T, Extra> {
  ...
  /** 会话来源，默认为 godseye / Conversation source, defaults to godseye */
...
export interface IMcpServer {
  ...
  /** Built-in MCP server managed by Gods Eye (hide edit/delete in UI) */
  builtin?: boolean;
}
