...
export interface IStartOnBootStatus {
  ...
  supported: boolean;
  /** Whether Gods Eye is currently configured to launch at login */
  enabled: boolean;
...
export const deepLink = {
  /** Emitted when app is opened via godseye:// protocol URL */
...
export interface IExtensionSettingsTab {
  ...
  name: string;
  icon?: string;
  /** godseye-asset:// local page or external https:// URL */
  entryUrl: string;
...
// CLI Installer API — auto-install Gods Eye CLI from the desktop app
export interface ICliInstallerStatus {
  /** Whether the godseye CLI is installed and available in PATH */
  installed: boolean;
...
export interface ICliInstallOutput {
...
export interface ICliSetupPrompt {
  /** Unique prompt ID for pairing answers */
  promptId: string;
...
export interface ICliSetupAnswer {
  /** Prompt ID to answer */
  promptId: string;
  /** User's answer */
  answer: string;
...
export const cliInstaller = {
  /** Check if Gods Eye CLI is installed */
  checkInstalled: bridge.buildProvider<IBridgeResponse<ICliInstallerStatus>, void>('cli-installer.check-installed'),
  /** Install Gods Eye CLI via npm */
  install: bridge.buildProvider<IBridgeResponse<{ success: boolean }>, { method?: 'npm' | 'pnpm' | 'yarn' }>(
    'cli-installer.install'
  ),
  /** Stream install output (stdout/stderr) */
  installOutput: bridge.buildEmitter<ICliInstallOutput>('cli-installer.install-output'),
  /** Run CLI setup/onboard wizard */
  runSetup: bridge.buildProvider<IBridgeResponse<{ success: boolean }>, void>('cli-installer.run-setup'),
  /** Setup prompt emitted when CLI asks a question */
  setupPrompt: bridge.buildEmitter<ICliSetupPrompt>('cli-installer.setup-prompt'),
  /** Send answer to a setup prompt */
  setupAnswer: bridge.buildProvider<void, ICliSetupAnswer>('cli-installer.setup-answer'),
  /** Send raw keypress to setup process stdin (arrow keys, enter, space) */
  setupKeypress: bridge.buildProvider<void, { data: string }>('cli-installer.setup-keypress'),
  /** Open godseye onboard in the native system terminal */
  openSetupInTerminal: bridge.buildProvider<void, void>('cli-installer.open-setup-terminal'),
