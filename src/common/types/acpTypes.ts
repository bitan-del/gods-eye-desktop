...
export type AcpBackendAll =
  ...
  | 'qoder' // Qoder CLI
  | 'godseye-gateway' // Gods Eye Gateway WebSocket
  | 'openclaw-gateway' // OpenClaw Gateway WebSocket (legacy)
...
export const ACP_BACKENDS_ALL: Record<AcpBackendAll, AcpBackendConfig> = {
  ...
  'godseye-gateway': {
    ...
    cliCommand: 'godseye',
    authRequired: false,
    enabled: true, // ✅ Gods Eye Gateway WebSocket mode
    supportsStreaming: true,
    acpArgs: ['gateway'], // godseye gateway command (for detection)
    skillsDirs: ['.godseye/skills'],
  },
  'openclaw-gateway': {
    id: 'openclaw-gateway',
    name: 'Gods Eye',
    cliCommand: 'openclaw',
    authRequired: false,
    enabled: false, // ❌ Hidden — OpenClaw is rebranded to Gods Eye; kept for backward compat with existing conversations
    supportsStreaming: true,
    acpArgs: ['gateway'],
  },
  nanobot: {
