...
import { GODSEYE_TIMESTAMP_REGEX } from '@/common/config/constants';

// 清理Gods Eye时间戳后缀，返回原始文件名
export function cleanGodsEyeTimestamp(fileName: string): string {
  return fileName.replace(GODSEYE_TIMESTAMP_REGEX, '$1');
}

// 从文件路径获取清理后的文件名（用于UI显示）
export function getCleanFileName(filePath: string): string {
  const fileName = filePath.split(/[\\/]/).pop() || '';
  return cleanGodsEyeTimestamp(fileName);
}
