...
/** GodsEyeModal 组件 Props */
export interface GodsEyeModalProps extends Omit<ModalProps, 'title' | 'footer'> {
 ...
 * ```tsx
 * // 基本用法 / Basic usage
 * <GodsEyeModal visible={true} onCancel={handleClose} header="标题">
 *   内容
 * </GodsEyeModal>
 *
 * // 预设尺寸 / Preset size
 * <GodsEyeModal visible={true} size="large" header="大型弹窗">
 *   内容
 * </GodsEyeModal>
 *
...
const GodsEyeModal: React.FC<GodsEyeModalProps> = ({
  children,
  size,
  header,
  footer,
  ...
  return (
    <Modal
      ...
      footer={null}
      onCancel={onCancel}
      className={`godseye-modal ${className}`}
      style={finalStyle}
      getPopupContainer={() => document.body}
    >
      <div className='godseye-modal-wrapper' style={{ borderRadius: borderRadiusVal }}>
        {renderHeader()}
        <div className='godseye-modal-body-content' style={bodyInlineStyle}>
          {children}
        </div>
        {renderFooter()}
      </div>
...
GodsEyeModal.displayName = 'GodsEyeModal';

export default GodsEyeModal;
