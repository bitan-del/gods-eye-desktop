...
interface ExtensionSettingsTabContentProps {
  /** godseye-asset:// local page URL or external https:// URL */
  entryUrl: string;
  /** Tab ID for keying */
  tabId: string;
 ...
 * - Local URLs (godseye-asset://) → sandboxed iframe with postMessage bridge.
 */
const ExtensionSettingsTabContent: React.FC<ExtensionSettingsTabContentProps> = ({
  ...
  // postMessage bridge for local iframe tabs (godseye-asset://)
  useEffect(() => {
    if (isExternalTab) return;
    ...
    const onMessage = async (event: MessageEvent) => {
      ...
      if (data.type === 'godseye:get-locale') {
        void postLocaleInit();
        return;
      }
