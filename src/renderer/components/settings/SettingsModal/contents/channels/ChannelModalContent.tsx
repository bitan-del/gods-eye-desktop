...
const ChannelModalContent: React.FC = () => {
  ...
  const handleToggleWhatsappPlugin = async (enabled: boolean) => {
    setWhatsappEnableLoading(true);
    try {
      if (enabled) {
        if (!whatsappPluginStatus?.hasToken) {
          Message.warning(t('settings.whatsapp.loginRequired', 'Please scan the QR code to connect WhatsApp first'));
          setWhatsappEnableLoading(false);
          return;
        }
        const result = await channel.enablePlugin.invoke({
        ...
        if (result.success) {
          ...
          Message.error(result.msg || t('settings.whatsapp.enableFailed', 'Failed to enable WhatsApp plugin'));
        }
      } else {
        const result = await channel.disablePlugin.invoke({
          pluginId: 'whatsapp_default',
        ...
        if (result.success) {
          ...
          Message.error(result.msg || t('settings.whatsapp.disableFailed', 'Failed to disable WhatsApp plugin'));
        }
      }
    } catch (error: any) {
      Message.error(error.message);
  ...
  const channels: ChannelConfig[] = useMemo(() => {
    const telegramChannel: ChannelConfig = {
      ...
      description: t('settings.channels.telegramDesc', 'Chat with Gods Eye assistant via Telegram'),
      status: 'active',
    ...
    const larkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.larkDesc', 'Chat with Gods Eye assistant via Lark or Feishu'),
      status: 'active',
    ...
    const dingtalkChannel: ChannelConfig = {
      ...
      description: t('settings.channels.dingtalkDesc', 'Chat with Gods Eye assistant via DingTalk'),
      status: 'active',
    ...
    const weixinChannel: ChannelConfig = {
      ...
      description: t('settings.channels.weixinDesc', 'Chat with Gods Eye assistant via WeChat'),
      status: 'active',
    ...
    const whatsappChannel: ChannelConfig = {
      ...
      description: t('settings.channels.whatsappDesc', 'Chat with Gods Eye assistant via WhatsApp'),
      status: 'active',
      enabled: whatsappPluginStatus?.enabled || false,
      disabled: whatsappEnableLoading,
      isConnected: whatsappPluginStatus?.connected || false,
      defaultModel: whatsappModelSelection.currentModel?.useModel,
      content: (
        <WhatsAppConfigForm
          pluginStatus={whatsappPluginStatus}
          modelSelection={whatsappModelSelection}
          onStatusChange={setWhatsappPluginStatus}
        />
      ),
    };
    ...
    const comingSoonChannels: ChannelConfig[] = [
      {
        ...
        description: t('settings.channels.slackDesc', 'Chat with Gods Eye assistant via Slack'),
      ...
      },
      {
        ...
        description: t('settings.channels.discordDesc', 'Chat with Gods Eye assistant via Discord'),
        status: 'coming_soon' as const,
        enabled: false,
        disabled: true,
        content: (
  ...
  const channelGuideText = t('settings.webui.featureChannelsDesc', {
    defaultValue: 'Connect Telegram, Lark, DingTalk, and WhatsApp to interact with Gods Eye from IM apps.',
  });
