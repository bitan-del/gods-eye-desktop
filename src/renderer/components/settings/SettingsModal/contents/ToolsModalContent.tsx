...
import GodsEyeSelect from '@/renderer/components/base/GodsEyeSelect';
...
const SPEECH_TO_TEXT_CONFIG_CHANGED_EVENT = 'godseye:speech-to-text-config-changed';
    ...
    <div className='px-[12px] md:px-[32px] py-[24px] bg-2 rd-12px md:rd-16px border border-border-2'>
      ...
      <Form layout='horizontal' labelAlign='left' className='space-y-12px'>
        <Form.Item label={t('settings.speechToTextProvider')}>
          <GodsEyeSelect value={config.provider} onChange={handleProviderChange}>
            <GodsEyeSelect.Option value='openai'>{t('settings.speechToTextProviderOpenAI')}</GodsEyeSelect.Option>
            <GodsEyeSelect.Option value='deepgram'>{t('settings.speechToTextProviderDeepgram')}</GodsEyeSelect.Option>
          </GodsEyeSelect>
        </Form.Item>
...
const ToolsModalContent: React.FC = () => {
  ...
  const syncMcpServerEnv = useCallback(
    async (model: Partial<IConfigStorageRefer['tools.imageGenerationModel']>) => {
      ...
      if (model.platform) {
        env.GODSEYE_IMG_PLATFORM = model.platform;
      } else {
        delete env.GODSEYE_IMG_PLATFORM;
      }
      if (model.baseUrl) {
        env.GODSEYE_IMG_BASE_URL = model.baseUrl;
      } else {
        delete env.GODSEYE_IMG_BASE_URL;
      }
      if (model.apiKey) {
        env.GODSEYE_IMG_API_KEY = model.apiKey;
      } else {
        delete env.GODSEYE_IMG_API_KEY;
      }
      if (model.useModel) {
        env.GODSEYE_IMG_MODEL = model.useModel;
      } else {
        delete env.GODSEYE_IMG_MODEL;
      }
                ...
                {imageGenerationModelList.length > 0 ? (
                  ...
                  >
                    {imageGenerationModelList.map(({ model, ...platform }) => (
                      <GodsEyeSelect.OptGroup label={platform.name} key={platform.id}>
                        {model.map((modelName) => (
                          <GodsEyeSelect.Option key={platform.id + modelName} value={platform.id + '|' + modelName}>
                            {modelName}
                          </GodsEyeSelect.Option>
                        ))}
                    ...
                    <Tooltip
                      content={
                        <div>
                          ...
                          <a
                            href='https://github.com/bitan-del/gods-eye-desktop/wiki/Gods Eye-Image-Generation-Tool-Model-Configuration-Guide'
                            target='_blank'
