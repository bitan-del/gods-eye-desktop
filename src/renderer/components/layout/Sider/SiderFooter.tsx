...
import { ArrowCircleLeft, Moon, Robot, SettingTwo, SunOne } from '@icon-park/react';
  ...
  return (
    <div className='shrink-0 sider-footer mt-auto pt-4px pb-8px'>
      <div className={classNames('flex', collapsed ? 'flex-col gap-2px' : 'items-center gap-2px')}>
        {/* Jarvis Mode button */}
        <Tooltip {...siderTooltipProps} content='Jarvis' position='right'>
          <div
            onClick={onJarvisClick}
            className={classNames(
              'h-40px flex items-center rd-0.5rem cursor-pointer transition-colors',
              collapsed ? 'w-full justify-center' : 'shrink-0 justify-center w-40px',
              isMobile && 'sider-footer-btn-mobile',
              {
                'bg-[rgba(3,169,244,0.12)] text-[#4fc3f7]': isJarvis,
                'hover:bg-fill-2 text-t-secondary hover:text-t-primary active:bg-fill-3': !isJarvis,
              }
            )}
            aria-label='Jarvis'
          >
