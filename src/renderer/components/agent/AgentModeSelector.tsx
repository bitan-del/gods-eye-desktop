...
const AgentModeSelector: React.FC<AgentModeSelectorProps> = ({
  ...
  useEffect(() => {
    ...
    ConfigStorage.get('acp.cachedModes')
      .then((cachedModesMap) => {
        if (cancelled) return;
        const sessionModes = cachedModesMap?.[backend];
        ...
        return ConfigStorage.get('acp.cachedConfigOptions').then((cached) => {
          ...
          if (Array.isArray(options)) {
            const extracted = extractModesFromConfigOptions(options as AcpSessionConfigOption[]);
            if (extracted.length > 0) {
              setCachedModes(extracted);
            }
          }
        });
      })
  ...
  // Priority: cachedModes (from cache) > getAgentModes (static fallback)
  const modes = useMemo(() => {
