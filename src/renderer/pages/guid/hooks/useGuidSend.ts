...
export const useGuidSend = (deps: GuidSendDeps): GuidSendResult => {
  ...
  const handleSend = useCallback(async () => {
    ...
    // Gods Eye / OpenClaw Gateway path
    if (selectedAgent === 'openclaw-gateway' || selectedAgent === 'godseye-gateway') {
      ...
      try {
        ...
        if (!conversation || !conversation.id) {
          alert('Failed to create Gods Eye conversation. Please ensure the Gods Eye Gateway is running.');
          return;
        }
        ...
        alert(`Failed to create Gods Eye conversation: ${errorMessage}`);
        throw error;
      }
      return;
    }
    ...
    if (selectedAgent === 'aionrs') {
      try {
        ...
        if (!conversation || !conversation.id) {
          alert('Failed to create Gods Eye CLI conversation. Please ensure aionrs is installed.');
          return;
        }
        ...
        const errorMessage = error instanceof Error ? error.message : String(error);
        alert(`Failed to create Gods Eye CLI conversation: ${errorMessage}`);
        throw error;
      }
      return;
    }
