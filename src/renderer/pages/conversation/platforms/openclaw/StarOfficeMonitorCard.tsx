...
const MONITOR_URL_STORAGE_KEY = 'godseye.openclaw.monitorUrl';
const STAR_OFFICE_URL_KEY = 'godseye.starOffice.url';
  ...
  const handleConfirm = useCallback(() => {
    ...
    try {
      ...
      onOpenUrl(normalized, {
        title: t('starOffice.monitor.title', { defaultValue: 'Gods Eye Live Monitor' }),
      });
      setVisible(false);
    } catch {
          ...
          <div className='rounded-12px border border-3 bg-2 p-12px'>
            ...
            <div className='mt-6px text-12px leading-18px text-t-secondary'>
              {t('starOffice.monitor.visualDesc', {
                defaultValue:
                  'Star Office is a visual companion for Gods Eye. It turns chat-side status into a live, interactive monitor view.',
              })}
            </div>
                  ...
                  <div className='text-center'>
                    ...
                    <div className='mt-2px text-11px text-t-secondary'>
                      {t('starOffice.monitor.visualFallbackDesc', {
                        defaultValue: 'Gods Eye chat status becomes a visual office scene.',
                      })}
                    </div>
                  </div>
                </div>
            ...
            <div className='mt-10px flex items-center gap-6px text-12px text-t-primary flex-wrap'>
              <span className='rounded-full border border-3 bg-1 px-8px py-4px'>
                {t('starOffice.monitor.visualStepChat', { defaultValue: 'Gods Eye Chat' })}
              </span>
            ...
            {detectState === 'ready' ? (
              ...
              <div className='mt-10px flex flex-wrap items-center gap-8px'>
                <Button type='primary' className='!rounded-10px' onClick={handleAskOpenClawInstall}>
                  {t('starOffice.monitor.installWithOpenClaw', { defaultValue: 'Install with Gods Eye' })}
                </Button>
                <Button
                  size='mini'
                  type='outline'
