...
import { useCliSetupWizard } from './hooks/useCliSetupWizard';
...
const Main = () => {
  const { ready } = useAuth();
  const { wizardVisible, closeWizard, initialStep } = useCliSetupWizard();
  ...
  return (
    <>
      ...
      />
      <SetupWizardModal visible={wizardVisible} onClose={closeWizard} initialStep={initialStep} />
    </>
  );
};
