...
const ASSET_PROTOCOL_PREFIX = 'godseye-asset://asset/';
 ...
 * - In Electron dev / any HTTP(S)-served renderer: keep `godseye-asset://` because direct `file://` is blocked.
 * - In Electron packaged / local-protocol renderers: convert `godseye-asset://asset/{path}` to `file://` for reliable image loading.
 * - In a regular browser (WebUI): convert `godseye-asset://asset/{path}` to `/api/ext-asset?path={encodedPath}`.
 *
