...
import { GODSEYE_ASSET_PROTOCOL } from '@process/extensions';
...
const isE2ETestMode = process.env.GODSEYE_E2E_TEST === '1';
const skipSingleInstanceLock = isE2ETestMode || process.env.GODSEYE_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[Gods Eye] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[Gods Eye] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
// Register godseye-asset:// as a privileged scheme BEFORE app.whenReady().
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[Gods Eye] Creating main window...');
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[Gods Eye] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[Gods Eye] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[Gods Eye] Renderer did-finish-load');
      showWindow();
    });
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[Gods Eye] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[Gods Eye] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[Gods Eye] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[Gods Eye] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[Gods Eye] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[Gods Eye] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[Gods Eye] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[Gods Eye] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[Gods Eye] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[Gods Eye] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[Gods Eye] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[Gods Eye] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[Gods Eye] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[Gods Eye:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  // Register godseye-asset:// protocol handler.
  // Converts godseye-asset://asset/C:/path/to/file.svg → file:///C:/path/to/file.svg
  // and serves the local file through Electron's net module.
  protocol.handle(GODSEYE_ASSET_PROTOCOL, (request) => {
    ...
    if (!fs.existsSync(filePath)) {
      console.warn(`[godseye-asset] File not found: ${request.url} -> ${filePath}`);
    }
  ...
  try {
    ...
    console.error('[Gods Eye] Failed to restore zoom factor:', error);
...
// Register godseye:// as the default protocol client
...
// macOS: handle godseye:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[Gods Eye] App initialization failed:', error);
    app.quit();
  });
...
app.on('before-quit', async () => {
  console.log('[Gods Eye] before-quit');
  setIsQuitting(true);
  isExplicitQuit = true;
  destroyTray();
  ...
  const timeout = new Promise<void>((resolve) => {
    setTimeout(() => {
      console.warn('[Gods Eye] Cleanup timed out after 10s, forcing quit');
      resolve();
    }, 10000);
  });
...
app.on('will-quit', () => {
  console.log('[Gods Eye] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[Gods Eye] quit (exitCode=${exitCode})`);
});
