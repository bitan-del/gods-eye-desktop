...
import { getGodseyeMcpStdioConfig } from '@process/services/mcpServices/godseyeMcpServiceSingleton';
...
> {
  ...
  private async getMcpServers(): Promise<Record<string, UiMcpServerConfig>> {
    try {
      ...
      if (this.teamMcpStdioConfig && this.teamMcpStdioConfig.command) {
        ...
        // so Gemini can call godseye_create_team / godseye_navigate after the user requests a Team
        // or explicitly approves Team for an exceptionally hard task.
        // GODSEYE_MCP_BACKEND tells the stdio bridge this is a gemini agent.
        const godseyeStdioConfig = getGodseyeMcpStdioConfig();
        if (godseyeStdioConfig) {
          const godseyeEnvObj: Record<string, string> = {};
          for (const { name, value } of godseyeStdioConfig.env || []) {
            godseyeEnvObj[name] = value;
          }
          godseyeEnvObj['GODSEYE_MCP_BACKEND'] = 'gemini';
          mcpConfig[godseyeStdioConfig.name] = {
            command: godseyeStdioConfig.command,
            args: godseyeStdioConfig.args || [],
            env: godseyeEnvObj,
          };
          mainLog('[GeminiAgentManager]', 'getMcpServers: injected godseye team-guide MCP server');
        }
      }
  ...
  private tryAutoApprove(content: IMessageToolGroup['content'][number]): boolean {
    ...
    // Team MCP servers (godseye-team-*) are always auto-approved regardless of mode
    if (type === 'mcp') {
      const serverName = (content.confirmationDetails as { serverName?: string })?.serverName ?? '';
      if (serverName.startsWith('godseye-team-')) {
        console.log(
