...
import { GODSEYE_FILES_MARKER } from '@/common/config/constants';
...
${collectedResponses.join('\n')}`;
  ...
  private async handleSignalEvent(v: IResponseMessage, backend: AcpBackend): Promise<void> {
    ...
    if (v.type === 'acp_permission') {
      ...
      // Auto-approve team MCP tools — internal tools provided by Gods Eye.
      const toolTitle = toolCall.title || '';
      if (toolTitle.includes('godseye-team') && options.length > 0) {
        const autoOption = options[0];
        setTimeout(() => {
          void this.confirm(v.msg_id, toolCall.toolCallId || v.msg_id, autoOption);
        }, 50);
      ...
      channelEventBus.emitAgentMessage(this.conversation_id, {
        ...
        conversation_id: this.conversation_id,
        msg_id: v.msg_id,
        data: 'Permission required. Please open Gods Eye and confirm the pending request in the conversation panel.',
      });
      return;
    }
  ...
  async sendMessage(data: {
    ...
    try {
      ...
      if (data.msg_id && data.content) {
        ...
        if (contentToSend.includes(GODSEYE_FILES_MARKER)) {
          contentToSend = contentToSend.split(GODSEYE_FILES_MARKER)[0].trimEnd();
        }
