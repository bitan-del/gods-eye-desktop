...
const DUMMY_BCRYPT_PASSWORD = 'godseye-auth-dummy-password';
...
export class AuthService {
   ...
   *
   * 复用 Web 登录 token (audience: godseye-webui)
   *
  ...
  public static async verifyWebSocketToken(token: string): Promise<TokenPayload | null> {
    try {
      ...
      const decoded = jwt.verify(token, await this.getJwtSecret(), {
        issuer: 'godseye',
        audience: 'godseye-webui', // 使用与 Web 登录相同的 audience
      }) as RawTokenPayload;
