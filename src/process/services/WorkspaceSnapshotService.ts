...
export class WorkspaceSnapshotService {
  ...
  /**
   * Remove leftover `godseye-snapshot-*` directories from previous sessions
  ...
  static async cleanupStaleSnapshots(): Promise<void> {
    ...
    const stale = entries.filter((name) => name.startsWith('godseye-snapshot-'));
  ...
  private async createWorkingTreeSnapshot(workspacePath: string): Promise<string> {
    const gitdir = path.join(os.tmpdir(), `godseye-snapshot-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`);
    ...
    await execFileAsync(
      ...
      [
        ...
        'user.name=Gods Eye',
        '-c',
        'user.email=snapshot@godseye.local',
        'commit',
        '--allow-empty',
        '-m',
        'baseline',
