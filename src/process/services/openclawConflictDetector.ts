 ...
 * as Gods Eye Channels, and warns the user about the conflict.
 */
...
/**
 * Check if OpenClaw Lark channel conflicts with Gods Eye credentials
 */
export function detectLarkConflict(godseyeAppId: string): ConflictInfo | null {
  ...
  for (const [accountName, account] of Object.entries(feishu.accounts)) {
    if (account.enabled && account.appId === godseyeAppId) {
      console.warn(
        `[OpenClawConflictDetector] Lark conflict detected: OpenClaw account "${accountName}" uses same appId: ${godseyeAppId}`
      );
      return {
        platform: 'lark',
        openclawEnabled: true,
        credentialMatch: true,
        openclawCredential: godseyeAppId,
      };
    }
  }
...
/**
 * Check if OpenClaw Telegram channel conflicts with Gods Eye credentials
 */
export function detectTelegramConflict(godseyeBotToken: string): ConflictInfo | null {
  const config = readOpenClawConfig();
...
export function getConflictResolutionSteps(platform: 'lark' | 'telegram'): string[] {
  ...
  return [
    `Detected conflict: Gods Eye CLI ${platformName} channel is using the same credentials as Gods Eye Desktop.`,
    ``,
    `This means messages are being handled by Gods Eye CLI, not Gods Eye Desktop Channels.`,
    `Switching agents in Gods Eye will have no effect.`,
    ``,
    `To fix this, choose one:`,
    ``,
    `Option 1: Disable Gods Eye CLI ${platformName} channel`,
    `  - Edit: ${configPath}`,
    ...
    `  - Configure it in Gods Eye Channels`,
    `  - Keep Gods Eye CLI ${platformName} channel for other use`,
    ``,
    `Option 3: Use Gods Eye CLI for ${platformName}`,
    `  - Disable ${platformName} in Gods Eye Channels`,
    `  - Use Gods Eye CLI native ${platformName} integration`,
  ];
}
