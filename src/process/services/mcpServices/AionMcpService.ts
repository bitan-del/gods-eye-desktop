 ...
 * (out/main/godseye-mcp-stdio.js) bridges Claude CLI <-> TCP, matching the same
...
/** Allowed route patterns that godseye_navigate may redirect to */
...
/**
 * Resolve the directory containing the godseye-mcp-stdio.js bundle.
...
export class AionMcpService {
  ...
  getStdioConfig(): StdioMcpConfig {
    const scriptPath = path.join(resolveAionMcpDir(), 'godseye-mcp-stdio.js');
    return {
      name: 'godseye-team-guide',
      command: 'node',
      args: [scriptPath],
      env: [
        { name: 'GODSEYE_MCP_PORT', value: String(this._port) },
        { name: 'GODSEYE_MCP_TOKEN', value: this.authToken },
      ],
    };
  }
  ...
  private handleTcpConnection(socket: net.Socket): void {
    const reader = createTcpMessageReader(async (msg) => {
      const request = msg as {
        ...
        auth_token?: string;
        /** Backend type of the calling agent, injected by godseye-mcp-stdio via GODSEYE_MCP_BACKEND env var */
        backend?: string;
      };
  ...
  private async handleToolCall(toolName: string, args: Record<string, unknown>, backend?: string): Promise<string> {
    switch (toolName) {
      case 'godseye_create_team':
        return this.handleCreateTeam(args, backend);
      case 'godseye_navigate':
  ...
  private async handleCreateTeam(args: Record<string, unknown>, backend?: string): Promise<string> {
    ...
    // Use system-injected backend (from GODSEYE_MCP_BACKEND env var) as the authoritative agent type.
    ...
    return JSON.stringify({
      ...
      status: 'team_created',
      next_step: `Tell the user the team has been created, then call godseye_navigate with route "${route}" to take them to the team page.`,
    });
  }
