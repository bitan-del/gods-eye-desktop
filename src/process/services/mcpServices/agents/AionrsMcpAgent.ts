 ...
 * Maps to Gods Eye transport types (snake_case)
 */
...
/**
 * Map aionrs transport type (kebab-case) to Gods Eye transport type
 */
function toGodsEyeTransportType(aionrsType: AionrsTransportType): IMcpServerTransport['type'] {
  if (aionrsType === 'streamable-http') return 'streamable_http';
  return aionrsType;
}

/**
 * Map Gods Eye transport type to aionrs transport type (kebab-case)
 */
...
/**
 * Convert an aionrs server config entry to an Gods Eye IMcpServer
 */
function toMcpServer(name: string, config: AionrsServerConfig): IMcpServer {
  const transportType = toGodsEyeTransportType(config.transport);
  const now = Date.now();
...
/**
 * Convert an Gods Eye IMcpServer to an aionrs server config entry
 */
...
export class AionrsMcpAgent extends AbstractMcpAgent {
  ...
  getSupportedTransports(): string[] {
    // aionrs supports stdio, sse, streamable-http (mapped to streamable_http in Gods Eye)
