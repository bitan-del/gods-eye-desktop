...
/**
 * Main database class for Gods Eye
 * Uses a pluggable ISqliteDriver for SQLite operations
 */
export class GodsEyeDatabase {
  ...
  /**
   * Create a new GodsEyeDatabase instance with corruption recovery.
   * This is the only way to obtain an instance — the constructor is private.
   */
  static async create(dbPath: string): Promise<GodsEyeDatabase> {
    ...
    try {
      ...
      failedDriver = driver;
      const instance = new GodsEyeDatabase(driver);
      instance.initialize();
      return instance;
    } catch (error) {
   ...
   * Prefer using dedicated methods on GodsEyeDatabase where possible.
   */
...
let dbInstancePromise: Promise<GodsEyeDatabase> | null = null;
// Synchronous reference to the resolved instance — used for safe close on exit
let dbResolved: GodsEyeDatabase | null = null;

function resolveDbPath(): string {
  return path.join(getDataPath(), 'godseye.db');
}

export function getDatabase(): Promise<GodsEyeDatabase> {
  if (!dbInstancePromise) {
    dbInstancePromise = GodsEyeDatabase.create(resolveDbPath()).then((db) => {
      dbResolved = db;
      return db;
    });
  }
