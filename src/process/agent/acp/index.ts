...
export class AcpAgent {
  ...
  private async createOrResumeSession(): Promise<void> {
    ...
    // Derive teamId from injected team MCP server name (format: godseye-team-<teamId>)
    // Only emit MCP status events when running inside a team session.
    const teamMcpName = this.extra.teamMcpStdioConfig?.name;
    const teamId = teamMcpName?.startsWith('godseye-team-') ? teamMcpName.slice('godseye-team-'.length) : undefined;
  ...
  private async loadBuiltinSessionMcpServers(): Promise<AcpSessionMcpServer[]> {
    try {
      ...
      // GODSEYE_MCP_BACKEND env var tells the stdio bridge which backend this agent is,
      // so godseye_create_team automatically creates a team with the correct agent type.
      if (!this.extra.teamMcpStdioConfig && TEAM_GUIDE_ALLOWED_BACKENDS.has(this.extra.backend)) {
        ...
        if (godseyeStdioConfig) {
          const configWithBackend = {
            ...godseyeStdioConfig,
            env: [...godseyeStdioConfig.env, { name: 'GODSEYE_MCP_BACKEND', value: this.extra.backend }],
          };
      ...
      const tId =
        typeof mcpName === 'string' && mcpName.startsWith('godseye-team-')
          ? mcpName.slice('godseye-team-'.length)
          : undefined;
      if (tId) {
