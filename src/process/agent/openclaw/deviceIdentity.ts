...
// Gods Eye uses ~/.godseye/identity/device.json; fall back to ~/.openclaw for migrated installs
const GODSEYE_DIR = path.join(os.homedir(), '.godseye', 'identity');
const LEGACY_DIR = path.join(os.homedir(), '.openclaw', 'identity');

function resolveIdentityDir(): string {
  const override = process.env.GODSEYE_STATE_DIR?.trim() || process.env.OPENCLAW_STATE_DIR?.trim();
  if (override) {
    return path.join(override, 'identity');
  }
  // Prefer .godseye if it exists, otherwise fall back to .openclaw
  if (fs.existsSync(path.join(GODSEYE_DIR, 'device.json'))) {
    return GODSEYE_DIR;
  }
  if (fs.existsSync(path.join(LEGACY_DIR, 'device.json'))) {
    return LEGACY_DIR;
  }
  // Default to .godseye for new installs
  return GODSEYE_DIR;
}

const DEFAULT_FILE = path.join(resolveIdentityDir(), 'device.json');
