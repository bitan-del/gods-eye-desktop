...
// Config file paths — check Gods Eye first, then OpenClaw as fallback
const GODSEYE_STATE_DIR = path.join(os.homedir(), '.godseye');
const DEFAULT_STATE_DIR = path.join(os.homedir(), '.openclaw');
const CONFIG_FILENAME = 'godseye.json';
const LEGACY_CONFIG_FILENAMES = ['openclaw.json', 'clawdbot.json', 'moltbot.json', 'moldbot.json'];
...
function resolveStateDir(): string {
  ...
  // Check Gods Eye directory first
  if (fs.existsSync(GODSEYE_STATE_DIR)) {
    return GODSEYE_STATE_DIR;
  }

  // Fall back to OpenClaw directory
...
function findConfigPath(): string | null {
  const override = process.env.GODSEYE_CONFIG_PATH?.trim() || process.env.OPENCLAW_CONFIG_PATH?.trim();
  if (override) {
    return resolveUserPath(override);
  }
