...
export const resolveWebUIPort = (
  ...
  const envPort = parsePortValue(process.env.GODSEYE_PORT ?? process.env.PORT);
...
export const resolveRemoteAccess = (config: WebUIUserConfig, isRemoteMode: boolean): boolean => {
  const envRemote = parseBooleanEnv(process.env.GODSEYE_ALLOW_REMOTE || process.env.GODSEYE_REMOTE);
  const hostHint = process.env.GODSEYE_HOST?.trim();
