...
/**
 * Strip npx-specific flags that bundled bun's `bun x` does not accept.
 * `-y`/`--yes` are npx auto-confirm flags; `--prefer-offline` is npx cache hint.
 * bun x auto-installs without prompting and manages its own cache, so these are
 * safe to drop.
 */
...
/**
 * Resolve the launcher command used for npx-style invocations.
 *
 * The fork ships a bundled bun runtime so users don't need a global Node/npm
 * install.  Callers combine this with `['x', '--bun', <package>, ...args]` to
 * execute npx-style packages.  When the bundled binary is missing we fall back
 * to a bare `bun` command so the app can still work if the user has bun on
 * their PATH.
 */
...
export async function logEnvironmentDiagnostics(): Promise<void> {
  try {
    ...
    const lines: string[] = [
      `${ENV_TAG} ${ENV_DIVIDER}`,
      `${ENV_TAG}   Gods Eye v${appVersion} (${mode})`,
    ...
    console.warn('[Gods Eye:env] Failed to collect environment diagnostics:', error);
  }
}
