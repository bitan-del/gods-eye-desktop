...
export function getElectronConfigCandidatePaths(): string[] {
  ...
  if (process.platform === 'darwin') {
    return [
      path.join(home, '.godseye-config', 'godseye-config.txt'),
      path.join(home, '.godseye-config-dev', 'godseye-config.txt'),
    ];
  }
  if (process.platform === 'win32') {
    const appData = process.env.APPDATA ?? path.join(home, 'AppData', 'Roaming');
    return [
      path.join(appData, 'Gods Eye', 'config', 'godseye-config.txt'),
      path.join(appData, 'Gods Eye-Dev', 'config', 'godseye-config.txt'),
    ];
  }
  // Linux and other platforms
  return [
    path.join(home, '.config', 'Gods Eye', 'config', 'godseye-config.txt'),
    path.join(home, '.config', 'Gods Eye-Dev', 'config', 'godseye-config.txt'),
  ];
}
...
export async function migrateFromElectronConfig(configStore: ConfigStore): Promise<void> {
  try {
    ...
    if (Object.keys(sourceData).length === 0) {
      console.warn('[Gods Eye] Config migration: source file appears empty or corrupted, will retry next startup');
      return;
    }
    ...
    console.log('[Gods Eye] Config migrated from Electron desktop config:', sourcePath);
  } catch (error) {
    console.warn('[Gods Eye] Config migration from Electron failed:', error);
  }
}
 ...
 * @param sourcePath - absolute path to an godseye-config.txt file
...
export async function importConfigFromFile(
  ...
  try {
    ...
    if (!path.isAbsolute(sourcePath)) {
      ...
      console.warn('[Gods Eye] IMPORT_CONFIG_FROM: relative path provided, resolving to:', resolved);
      sourcePath = resolved;
    }

    const sourceData = decodeConfigFile(sourcePath);
    if (Object.keys(sourceData).length === 0) {
      console.warn('[Gods Eye] IMPORT_CONFIG_FROM: file is missing, empty, or corrupted:', sourcePath);
      return;
    }
    ...
    console.log('[Gods Eye] Config imported from:', sourcePath, '(overwrite:', overwrite, ')');
  } catch (error) {
    console.warn('[Gods Eye] IMPORT_CONFIG_FROM failed:', error);
  }
}
