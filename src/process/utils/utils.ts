...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'godseye');
};
 ...
 * Release builds use ~/.godseye; dev builds use ~/.godseye-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.godseye，Dev 模式使用 ~/.godseye-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'godseye');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.godseye'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.godseye-config; dev builds use ~/.godseye-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.godseye-config，Dev 模式使用 ~/.godseye-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.godseye-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[Gods Eye] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[Gods Eye] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[Gods Eye] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[Gods Eye] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
