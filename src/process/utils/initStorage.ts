...
const STORAGE_PATH = {
  config: 'godseye-config.txt',
  chatMessage: 'godseye-chat-message.txt',
  chat: 'godseye-chat.txt',
  env: '.godseye-env',
...
const migrateLegacyData = async () => {
  ...
  try {
    ...
    const isNewDirEmpty =
      ...
      (() => {
        try {
          ...
          console.warn('[Gods Eye] Warning: Could not read new directory during migration check:', error);
      ...
      if (isVerified) {
        ...
        if (path.resolve(oldDir) !== path.resolve(newDir)) {
          try {
            ...
            console.warn('[Gods Eye] 原目录清理失败，请手动删除:', oldDir, cleanupError);
          }
        }
      }
    ...
    console.error('[Gods Eye] 数据迁移失败:', error);
  }
...
const dirConfig = envFile.getSync('godseye.dir');
...
const buildMessageListStorage = (conversation_id: string, dir: string) => {
  const fullName = path.join(dir, 'godseye-chat-history', conversation_id + '.txt');
  if (!existsSync(fullName)) {
    mkdirSync(path.join(dir, 'godseye-chat-history'));
  }
  return JsonFileBuilder<TMessage[]>(path.join(dir, 'godseye-chat-history', conversation_id + '.txt'));
};
  ...
  return {
    ...
    backup(conversation_id: string) {
      ...
      return storage.backup(
        path.join(dir, 'godseye-chat-history', 'backup', conversation_id + '_' + Date.now() + '.txt')
      );
    },
  };
};
...
const initBuiltinAssistantRules = async (): Promise<void> => {
  ...
  const resolveBuiltinDir = (dirPath: string): string => {
    ...
    console.warn(`[Gods Eye] Could not find builtin ${dirPath} directory, tried:`, candidates);
    return candidates[0];
  };
  ...
  if (existsSync(builtinSkillsDir)) {
    try {
      ...
      console.warn(`[Gods Eye] Failed to sync builtin skills directory:`, error);
    }
  }
      ...
      for (const [locale, ruleFile] of Object.entries(preset.ruleFiles)) {
        try {
          ...
          if (!existsSync(sourceRulesPath)) {
            console.warn(`[Gods Eye] Source rule file not found: ${sourceRulesPath}`);
            continue;
          }
          ...
          console.warn(`[Gods Eye] Failed to copy rule file ${ruleFile}:`, error);
        }
      }
    } else {
      ...
      for (const [locale, skillFile] of Object.entries(preset.skillFiles)) {
        try {
          ...
          if (!existsSync(sourceSkillsPath)) {
            console.warn(`[Gods Eye] Source skill file not found: ${sourceSkillsPath}`);
            continue;
          }
          ...
          console.warn(`[Gods Eye] Failed to copy skill file ${skillFile}:`, error);
        }
      }
    } else {
...
const ensureBuiltinMcpServers = async (): Promise<void> => {
  try {
    ...
    const buildEnvFromConfig = (cfg: typeof oldConfig): Record<string, string> => {
      ...
      if (cfg.platform) env.GODSEYE_IMG_PLATFORM = cfg.platform;
      if (cfg.baseUrl) env.GODSEYE_IMG_BASE_URL = cfg.baseUrl;
      if (cfg.apiKey) env.GODSEYE_IMG_API_KEY = cfg.apiKey;
      if (cfg.useModel) env.GODSEYE_IMG_MODEL = cfg.useModel;
      return env;
    };
    ...
    if (changed) {
      ...
      console.log('[Gods Eye] Built-in MCP servers ensured');
    }
    ...
    console.error('[Gods Eye] Failed to ensure built-in MCP servers:', error);
  }
};
...
const cleanupOrphanedHealthCheckConversations = async () => {
  try {
    ...
    if (deletedCount > 0) {
      console.log(`[Gods Eye] Cleaned up ${deletedCount} orphaned health-check conversation(s) on startup`);
    }
  } catch (error) {
    console.warn('[Gods Eye] Failed to cleanup orphaned health-check conversations:', error);
  }
};

const initStorage = async () => {
  const t0 = performance.now();
  const mark = (label: string) => console.log(`[Gods Eye:init] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[Gods Eye] Failed to initialize default MCP servers:', error);
  }
  ...
  try {
    ...
    console.error('[Gods Eye] Failed to initialize builtin assistants:', error);
  }
...
export const loadSkillsContent = async (enabledSkills: string[]): Promise<string> => {
  ...
  for (const skillName of enabledSkills) {
    ...
    try {
      ...
      console.warn(`[Gods Eye] Failed to load skill ${skillName}:`, error);
    }
  }
