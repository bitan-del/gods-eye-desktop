...
export async function resetPasswordCLI(username: string): Promise<void> {
  try {
    ...
    const dbPath = path.join(getDataPath(), 'godseye.db');
    ...
    if (!hasUsersResult.data) {
      ...
      log.info('');
      log.info('Please run Gods Eye at least once to initialize the database:');
      log.info('  godseye --webui');
      log.info('');
      log.info('Then you can reset the password using:');
      log.info('  godseye --resetpass <username>');
      process.exit(1);
    }
