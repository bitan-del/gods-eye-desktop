...
/**
 * Current Gods Eye extension API version.
 * Increment major for breaking changes, minor for new features, patch for fixes.
 */
export const GODSEYE_VERSION = getGodsEyeVersion();
...
/**
 * Get the Gods Eye version from package.json.
 * Falls back to '0.0.0' if not available.
 */
function getGodsEyeVersion(): string {
...
/**
 * Validate that an extension's engine requirements are satisfied by the current Gods Eye version.
 *
 * Checks:
 * 1. engine.godseye — does the running Gods Eye version satisfy the required range?
...
export function validateEngineCompatibility(extension: LoadedExtension): EngineValidationResult {
  ...
  // Check Gods Eye core version compatibility
  if (engine?.godseye) {
    if (!satisfiesVersion(GODSEYE_VERSION, engine.godseye)) {
      result.valid = false;
      result.issues.push(
        `Extension "${extension.manifest.name}" requires Gods Eye ${engine.godseye} but current version is ${GODSEYE_VERSION}`
      );
    }
  }
