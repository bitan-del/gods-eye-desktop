...
const DEBUG_ENABLED = process.env.GODSEYE_EXTENSION_DEBUG === '1' || process.env.GODSEYE_EXTENSION_DEBUG === 'true';
  ...
  for (const ext of extensions) {
    ...
    for (const plugin of plugins) {
      ...
      try {
        ...
        // Resolve icon path to absolute URL (godseye-asset://) for frontend
        let iconUrl = plugin.icon;
        if (plugin.icon && !plugin.icon.match(/^(https?:|data:|godseye-asset:|file:)/)) {
