...
export const GODSEYE_EXTENSIONS_PATH_ENV = 'GODSEYE_EXTENSIONS_PATH';
export const GODSEYE_STRICT_ENV_ENV = 'GODSEYE_STRICT_ENV';
export const EXTENSION_MANIFEST_FILE = 'godseye-extension.json';
...
export function getEnvExtensionsDirs(): string[] {
  const envPath = process.env[GODSEYE_EXTENSIONS_PATH_ENV];
  if (!envPath) return [];
 ...
 * Set GODSEYE_HUB_URL to prepend custom URLs (comma-separated, highest priority).
 * Example: GODSEYE_HUB_URL=http://localhost:3000/,http://staging.example.com/
 */
...
function resolveHubRemoteUrls(): string[] {
  const envUrls = process.env.GODSEYE_HUB_URL;
 ...
 *
 * Priority order:
 *   1. Environment variable (`GODSEYE_EXTENSIONS_PATH`) — highest
 *   2. User data dir (`~/.godseye/extensions`)
 *   3. Electron appData dir
 *
 * E2E test mode (`GODSEYE_E2E_TEST=1`) only scans env dirs for hermetic runs.
 */
export function getExtensionScanSources(): ExtensionScanSource[] {
  const sources: ExtensionScanSource[] = [];
  const seen = new Set<string>();
  const isE2ETest = process.env.GODSEYE_E2E_TEST === '1';
