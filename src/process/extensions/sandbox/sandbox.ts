...
/**
 * Callback invoked when the worker sends a UI-bound message via godseye.postToUI().
 */
...
export interface SandboxHostOptions {
  ...
  /** Callback for messages sent via godseye.postToUI() in the worker. */
  ...
  private handleMessage(msg: SandboxMessage): void {
    switch (msg.type) {
      ...
      case 'event': {
        ...
        if (name === 'ui-message') {
          // godseye.postToUI() — forward to the registered UI message handler
          this.options.onUIMessage?.(this.options.extensionName, payload);
        } else if (name.startsWith('ext:')) {
          // godseye.emitEvent() — forward to the extension event bus
