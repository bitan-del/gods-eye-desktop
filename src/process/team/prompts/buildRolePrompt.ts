// src/process/team/prompts/buildRolePrompt.ts
...
type BuildRolePromptParams = {
  ...
  /** Optional persistent memory injected from the Brain vault (Gods Eye extension). */
  agentMemory?: string;
};
...
export function buildRolePrompt(params: BuildRolePromptParams): string {
  const { agent, teammates, availableAgentTypes, renamedAgents, teamWorkspace, agentMemory } = params;
