...
3. **Output the table as a normal text message and END YOUR TURN.** Do NOT call \`godseye_create_team\` or any other tool (including ask_user) in this turn. Wait for the user to reply in their next message with explicit confirmation (e.g. "ok", "go ahead", "确认") before proceeding.
4. After user confirms → call \`godseye_create_team\`. The summary MUST include both the goal and the confirmed team configuration. (The system automatically sets the correct agent type — you do NOT need to pass agentType.)
5. After \`godseye_create_team\` returns → read the response carefully and follow the next_step instructions. Tell the user the team is created → immediately call \`godseye_navigate\` with the route from the response. **Both calls (create + navigate) are required.**
6. User declines or wants changes → adjust or proceed solo. Do not mention Team again unless the user asks.

### Tool constraint
Use **only** \`godseye_create_team\` and \`godseye_navigate\` for team operations. Do NOT use any built-in or other team/agent creation tools.`;
}

/**
 * Description for the godseye_create_team MCP tool.
 */
