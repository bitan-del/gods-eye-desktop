...
// Priority: 1) GODSEYE_FONTS_DIR env var, 2) skills/canvas-design relative path, 3) system fonts
function getFontDir() {
  const candidates = [
    process.env.GODSEYE_FONTS_DIR,
    path.join(__dirname, '../../canvas-design/canvas-fonts'),
    path.join(process.env.HOME || '', 'Library/Application Support/Gods Eye/config/skills/canvas-design/canvas-fonts'),
    path.join(process.env.APPDATA || '', 'Gods Eye/config/skills/canvas-design/canvas-fonts'),
  ].filter(Boolean);
