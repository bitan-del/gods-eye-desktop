 ...
 * the main process TCP server via GODSEYE_MCP_PORT environment variable.
 *
...
const GODSEYE_MCP_TOKEN = process.env.GODSEYE_MCP_TOKEN || undefined;
/** Backend type of the agent that owns this stdio bridge (e.g. 'claude', 'codex', 'gemini'). */
const GODSEYE_MCP_BACKEND = process.env.GODSEYE_MCP_BACKEND || '';
process.stderr.write(
  `[godseye-mcp-stdio] Script started. PID=${process.pid}, GODSEYE_MCP_PORT=${process.env.GODSEYE_MCP_PORT || 'unset'}, BACKEND=${GODSEYE_MCP_BACKEND || 'unset'}\n`
);
const GODSEYE_MCP_PORT = parseInt(process.env.GODSEYE_MCP_PORT || '0', 10);

if (!GODSEYE_MCP_PORT) {
  process.stderr.write('GODSEYE_MCP_PORT environment variable is required\n');
  process.exit(1);
}

if (!GODSEYE_MCP_TOKEN) {
  process.stderr.write('GODSEYE_MCP_TOKEN environment variable is required\n');
  process.exit(1);
}
...
const server = new McpServer({ name: 'godseye-team-guide', version: '1.0.0' }, { capabilities: { tools: {} } });
...
main().catch((err: unknown) => {
  process.stderr.write(`[godseye-mcp-stdio] Fatal error: ${err}\n`);
  process.exit(1);
});
