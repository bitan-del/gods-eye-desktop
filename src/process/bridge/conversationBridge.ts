...
export function initConversationBridge(
  ...
  const emitConversationListChanged = (
    ...
    ipcBridge.conversation.listChanged.emit({
      ...
      action,
      source: conversation.source || 'godseye',
    });
  };
  ...
  ipcBridge.conversation.remove.provider(async ({ id }) => {
    try {
      ...
      // If source is not 'godseye' (e.g., telegram), cleanup channel resources
      // 如果来源不是 godseye（如 telegram），需要清理 channel 相关资源
      if (source && source !== 'godseye') {
        try {
