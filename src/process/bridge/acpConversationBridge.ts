...
export function initAcpConversationBridge(workerTaskManager: IWorkerTaskManager): void {
  ...
  ipcBridge.acpConversation.getAvailableAgents.provider(async () => {
    try {
      ...
      // Insert at front so Gods Eye CLI appears before other agents (including Gemini)
      const aionrs = detectAionrs();
      if (aionrs.available) {
        const aionrsAgent = { backend: 'aionrs' as const, name: 'Gods Eye CLI', cliPath: aionrs.path };
        enriched.unshift({
          ...aionrsAgent,
