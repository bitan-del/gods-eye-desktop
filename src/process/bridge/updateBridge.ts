...
const DEFAULT_REPO = 'bitan-del/gods-eye';
const DEFAULT_USER_AGENT = 'Gods Eye';
...
const resolveRepo = (requestRepo?: string): string => {
  const envRepo = process.env.GODSEYE_GITHUB_REPO?.trim();
...
const sanitizeFileName = (name: string): string => {
  ...
  // Avoid empty names.
  return base || `Gods Eye-update-${Date.now()}`;
};
