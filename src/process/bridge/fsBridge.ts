...
import { GODSEYE_TIMESTAMP_SEPARATOR } from '@/common/config/constants';
...
export function initFsBridge(): void {
  ...
  ipcBridge.fs.createTempFile.provider(async ({ fileName }) => {
    try {
      ...
      if (fileExists) {
        ...
        const tempFileName = `${name}${GODSEYE_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
      ...
      for (const filePath of filePaths) {
        try {
          ...
          if (exists) {
            ...
            const newFileName = `${name}${GODSEYE_TIMESTAMP_SEPARATOR}${timestamp}${ext}`;
  ...
  ipcBridge.fs.enableSkillsMarket.provider(async () => {
    try {
      ...
      const skillDir = path.join(getAutoSkillsDir(), 'godseye-skills');
  ...
  // Skills Market: remove the godseye-skills builtin skill
...
/**
 * Read the bundled SKILL.md for godseye-skills from app resources.
 *
...
async function readBundledSkillsMarketMd(): Promise<string> {
  try {
    const fallbackPath = path.join(getBuiltinSkillsCopyDir(), 'godseye-skills', 'SKILL.md');
    return await fs.readFile(fallbackPath, 'utf-8');
  } catch (error) {
    console.warn('[fsBridge] Failed to read bundled godseye-skills SKILL.md:', error);
    return `---\nname: godseye-skills\ndescription: "Access the Gods Eye Skills registry — discover and download AI agent skills."\n---\n\n# Gods Eye Skills Registry\n\nFetch full instructions:\n\n\`\`\`bash\nmkdir -p ~/.config/godseye-skills\ncurl -s https://skills.gods-eye.org/SKILL.md > ~/.config/godseye-skills/SKILL.md\n\`\`\`\n\nThen read and follow the instructions in that file.\n`;
  }
}
